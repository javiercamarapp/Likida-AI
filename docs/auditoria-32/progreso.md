...
| 11:52 | **arquitectura.md entregado** (454 líneas) | Nota **4 = 4**. **ARQ-A3 REPRODUCIDO** con cifras contra Postgres 16 efímero + las 333 migraciones. De pilón ARQ-A6 y ARQ-A2 también. 1 nuevo: ARQ-A7 [ALTO] |
| 11:58 | Reproducción propia de ARQ-A3 antes de tocar nada | Postgres reiniciado sobre el data dir del auditor. `supabase/tests/0357_dedup_ejercicio_emisor.sql` escrito → **ROJO medido**: `total=2500.00 (esperado 5000)` |
| 12:03 | Arreglo: migración **0357**, `coalesce(rfc_emisor,'')` en la partición del folio | Test **VERDE**. La prueba vieja de la 0349 **sigue verde** |
| 12:06 | Mutación en el sentido contrario (partición por `id`, dedup apagado) | **ROJO**: `total=6400 (esperado 5700)`. La prueba guarda las dos direcciones, no es vacua |
| 12:10 | Suite completa · 1ª vuelta | **ROJA** por `migraciones_verificadas.test.ts`: toda migración necesita bloque o entrada en EXENTAS. Guardia del propio repo, con razón |
| 12:14 | Entrada EXENTAS para 0357 + test cableado en `ci-postgres.yml` | Sin esto la prueba SQL no corre en CI: el workflow lista cada archivo a mano |
| 12:18 | Suite completa · 2ª vuelta | 987 pasan, **1 roja: `cron/asistencia/route.test.ts`**, por **1 ms** |
| 12:24 | ¿Es mía? `git stash -u` → 5 corridas sobre HEAD pristino | **NO.** `10 · 10 · FALLA · 10 · 10` → **intermitente PREEXISTENTE, 1 de cada 5**. Margen con cero holgura. Se anota como hallazgo nuevo y **no se toca**: sus rubros dueños no se rotaron hoy |
| 12:28 | Compuerta final | `tsc` exit 0 · `lint` 0 errores/154 avisos · `vitest` 988/12,976 verdes en 2 de 3 corridas |
| 12:30 | Commit atómico del arreglo | **`f0645f9`** — ARQ-A3, citando el ID del hallazgo |
| 12:38 | Tablero generado, capturado **y mirado** | Mirarlo encontró lo que medir no: la tarjeta de compuerta decía «verde · 0 fallan» copiando la línea base y callaba el intermitente. **Corregido y recapturado.** Verificado en la imagen: 12 filas, suma 56, global 4.7, 3 píldoras «auditado hoy», 11 puntos dentro del `viewBox` |
| 12:42 | `00-SINTESIS.md` + `RESULTADO.md` escritos | Global **4.7 = 4.7**. 1 arreglo retenido, 0 revertidos, 4 críticos pendientes con razón |
